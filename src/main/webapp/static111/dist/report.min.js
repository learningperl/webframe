rate="0.00";
record = "<ul width='100%' id='itemContainer2' class ='itemContainer'> <li><span>项目</span><span>状态</span><span>用例数</span><span>通过率</span><span>时间</span><span>生成报表</span></li>";
modules = '{"title":{"text":"当前用例模块组成","subtext":"Powered By Perlly","x":"center"},"tooltip":{"trigger":"item","formatter":"{a} <br/>{b} : {c} ({d}%)"},"legend":{"orient":"vertical","left":"left","data":[';
stastic = "<ul width='100%' id='itemContainer1' class ='itemContainer'><li><span style=\"width: 40%\">模块</span><span style=\"width: 25%\">用例数</span><span style=\"width: 25%\">状态</span></li>";
...
function getInfo() {
	...
	$.ajax({
		...
		success : function(result) {
				...
				var obj = eval("(" + data + ")");
				if (obj["status"] == "200") {
					document.cookie = 'user=' + obj["nickname"] + "; path=/TestFrame;";
...
function refresh(name){
	document.cookie = 'filename=' + name + "; path=/TestFrame;";
		...
		if (xmlhttp.readyState == 4){
			if (xmlhttp.status == 200){
				...
				try {
					...
					if (obj != null)
						if (obj["status"] == "200") {
							rate = obj["result"][obj["result"].length-1]["passrate"];
							var size = obj["result"].length;
							if(size==1){
								modules += '"null"';
								modules += ']},"series":[{"name":"模块占比","type":"pie","radius":"55%","center":["50%","60%"],"data":[';
								modules += '{"value":1,"name":"null"}';
								modules += '],"itemstyle":{"emphasis":{"shadowblur":10,"shadowoffsetx":0,"shadowcolor":"rgba(0, 0, 0, 0.5)"}}}]}';
								
								stastic += "</ul>";
							}else{
								for(var i=0;i<size-1;i++){
									var obj1 = obj["result"][i];
									modules += '"'+obj1["name"]+'",';
								}
								...
								for(var i=0;i<size-1;i++){
									var obj1 = obj["result"][i];
									modules += '{"value":'+obj1["count"]+',"name":"'+obj1["name"]+'"},';
								}
								modules = modules.substring(0,modules.length-1);
								modules += '],"itemstyle":{"emphasis":{"shadowblur":10,"shadowoffsetx":0,"shadowcolor":"rgba(0, 0, 0, 0.5)"}}}]}';
								...
								for(var i=0;i<size-1;i++){
									var obj1 = obj["result"][i];
									var Pass = "";
									if(obj1["status"]=="FAIL" || obj1["status"]=="Fail")
										Pass = "<span style=\"width: 25%\"><span class='am-badge am-badge-danger'>Fail</span></span>";
									else
										Pass = "<span style=\"width: 25%\"><span class='am-badge am-badge-success'>Pass</span></span>";
									stastic +="<li><span style=\"width: 40%\">"+obj1["name"]+"</span><span style=\"width: 25%\">"+obj1["count"]+"</span>"+Pass+"</li>";
								}
							}
						}else{
							modules += '"null"';
...
function getRess() {
	...
	var url = "./getRess?filename=" + f + "&page=1";
	var data = "";
	xmlhttp.open("GET", url, true);
	xmlhttp.send();
	xmlhttp.onreadystatechange = function() {
		if (xmlhttp.readyState == 4){
					...
					if (obj != null)
						if (obj["status"] == "200") {
							...
							for(var i=0;i<size;i++){
								var obj1 = obj["result"][i];
								var Pass = "";
								if(obj1["passrate"]=="100.00")
									Pass = "<span><span class='am-badge am-badge-success'>Pass</span></span>";
								else
									Pass = "<span><span class='am-badge am-badge-danger'>Fail</span></span>";
								record +="<li><span>"+obj1["resname"]+"</span>"+Pass+"</span><span>"+obj1["casecount"]+"</span><span>"+obj1["passrate"]+"</span><span>"+obj1["endtime"]+"</span><span><a href='javascript:refresh(\""+obj1["resname"]+"\");'>查看</a></span></li>";
							}
						}
				} catch (e) {
			...
			record += "</ul>";
			init();
			document.getElementById("rate").innerHTML="用例"+f+" - 通过率";
			document.getElementById("modules").innerHTML="用例"+f+" - 模块";
			document.getElementById("stastic").innerHTML="用例"+f+" - 统计";
			document.getElementById("record").innerHTML="用例"+f+" - 历史记录";
		}
	};
}
...
var Demo = function() {
	...
	Basic = function(a) {
		this.elem = a;
		this.data = function() {
			return '{"tooltip":{"formatter":"{a} <br/>{b} : {c}%"},"series":[{"name":"Pass Rate","type":"gauge","detail":{"formatter":"{value}%","width":"90","height":"60"},"center":["50%","70%"],"radius":"120%","startAngle":180,"endAngle":0,"data":[{"value":'+rate+',"name":"通过率"}]}]}'
		};
		this.render = function() {
			$("#" + this.elem).height(370);
